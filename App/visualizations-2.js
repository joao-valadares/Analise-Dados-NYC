...
// 6. Distribuição de tarifas (Histograma agrupado)
function visualizeFareDistribution(data) {
    const container = d3.select('#fare-distribution');
    container.selectAll('*').remove();
    ...
    const ranges = ['0-10', '10-20', '20-30', '30-50', '50+'];
    const groupedData = d3.group(cleanData, d => d.fare_range);
    
    const chartData = ranges.map(range => {
        const rangeData = groupedData.get(range) || [];
        return {
            range: range,
            y2019: toNumber(rangeData.find(d => d.year === 2019)?.count || 0),
            y2020: toNumber(rangeData.find(d => d.year === 2020)?.count || 0)
        };
    });
    ...
    const rangeGroup = svg.selectAll('.range-group')
        .data(chartData)
        .enter().append('g')
        .attr('class', 'range-group')
        .attr('transform', d => `translate(${x0(d.range)},0)`);
    
    rangeGroup.selectAll('rect')
        ...
        .on('mouseover', function(event, d) {
            tooltip.transition().duration(200).style('opacity', .9);
            tooltip.html(`<h4>$${d.range} - ${d.year}</h4>
...
// 10. Distribuição de Passageiros
function visualizePassengerDistribution(data) {
    const container = d3.select('#passenger-distribution');
    container.selectAll('*').remove();
    ...
    const groupedData = d3.group(cleanData, d => d.passenger_count);
    
    const chartData = passengerCounts.map(count => {
        const countData = groupedData.get(count) || [];
        return {
            count,
            y2019: toNumber(countData.find(d => d.year === 2019)?.count || 0),
            y2020: toNumber(countData.find(d => d.year === 2020)?.count || 0)
        };
    });
    
    // Escalas
    const x0 = d3.scaleBand()
        .domain(passengerCounts.map(String))
        .rangeRound([0, width])
        .paddingInner(0.1);
    ...
    const countGroup = svg.selectAll('.count-group')
        .data(chartData)
        .enter().append('g')
        .attr('class', 'count-group')
        .attr('transform', d => `translate(${x0(String(d.count))},0)`);
    
    countGroup.selectAll('rect')
        .data(d => ['2019', '2020'].map(year => ({
            year,
            count: d.count,
            value: year === '2019' ? d.y2019 : d.y2020
        ...
        .on('mouseover', function(event, d) {
            tooltip.transition().duration(200).style('opacity', .9);
            tooltip.html(`<h4>${d.count} passageiro${d.count > 1 ? 's' : ''} - ${d.year}</h4>
                          <p><strong>Viagens:</strong> ${d3.format(',')(d.value)}</p>`)
                .style('left', (event.pageX + 10) + 'px')
                .style('top', (event.pageY - 28) + 'px');
        })
