...
// 1. Visualizar registros por mês
function visualizeRecordsByMonth(data) {
    const container = d3.select('#records-by-month');
    ...
    monthGroup.selectAll('rect')
        ...
        .on('mouseover', function(event, d) {
            ...
            tooltip.html(`<h4>${d.month} ${d.year}</h4>
...
// 4. Tendência mensal
function visualizeMonthlyTrend(data) {
    ...
    const cleanData = data.map(convertBigInt).map(d => ({
        ...
        trips: toNumber(d.trips),
        avg_fare: toNumber(d.avg_fare) || 0,
        total_revenue: toNumber(d.total_revenue) || 0
    })).filter(d => {
        // Filtrar apenas dados válidos
        return d.month >= 10 && d.month <= 12 &&
               !isNaN(d.trips) && d.trips > 0;
    });
    
    if (cleanData.length === 0) {
        console.error('Nenhum dado válido para tendência mensal');
        return;
    }
    ...
    // Obter apenas os meses que existem nos dados
    const availableMonths = [...new Set(cleanData.map(d => d.monthName))];
    ...
    const y = d3.scaleLinear()
        .domain([0, d3.max(cleanData, d => d.trips)])
        .nice()
        .range([height, 0]);
    ...
    // Linhas
    const line = d3.line()
        .defined(d => !isNaN(d.trips) && d.monthName !== undefined)
    ...
    groupedData.forEach((values, year) => {
        // Ordenar valores por mês para linha conectar corretamente
        ...
        // Pontos
        svg.selectAll(`.dot-trend-${year}`)
            .data(sortedValues)
            .enter().append('circle')
            .attr('class', `dot dot-trend-${year}`)
            ...
            .on('mouseover', function(event, d) {
                ...
                tooltip.transition().duration(200).style('opacity', .9);
                tooltip.html(`<h4>${d.label}</h4>
                              <p><strong>Viagens:</strong> ${d3.format(',')(d.trips)}</p>
                              <p><strong>Tarifa Média:</strong> $${d.avg_fare.toFixed(2)}</p>
                              <p><strong>Receita Total:</strong> $${d3.format(',')(d.total_revenue.toFixed(0))}</p>`)
                    .style('left', (event.pageX + 10) + 'px')
                    .style('top', (event.pageY - 28) + 'px');
            })
            .on('mouseout', function() {
...
window.Visualizations = {
    visualizeRecordsByMonth,
    visualizeHourlyPattern,
    visualizeWeeklyPattern,
    visualizeMonthlyTrend,
